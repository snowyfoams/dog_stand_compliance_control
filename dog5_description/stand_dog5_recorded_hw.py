...
This runner uses the model-positive angles observed with ``direction_check.py``::
...
class RecordedCrouchController(base.HardwareStandController):
    ...
    def cartesian_target(self, leg, elapsed):
        s, ds = self._cubic(elapsed, T_STAND)
        delta = self.final_foot[leg] - self.crouch_foot[leg]
        return self.crouch_foot[leg] + s * delta, ds * delta, s
    ...
    def compute_stand(self, q, qd, elapsed):
        ...
        for leg_index, leg in enumerate(LEGS):
            ...
            desired, desired_velocity, progress = self.cartesian_target(
                leg, elapsed
            )
            error = desired - foot
            ...
            # No feedforward step at the joint-to-Cartesian transition.  It
            # grows smoothly from zero to the configured support share.
            force[2] -= (
...
class RecordedStandSequence:
    """CROUCH -> gate -> STAND trajectory -> tracking gate -> HOLD."""
    ...
    for leg_index, leg in enumerate(LEGS):
        ...
        for progress in np.linspace(0.05, 1.0, 20):
            target = (
                ...
                + progress
                * (controller.final_foot[leg] - controller.crouch_foot[leg])
            )
            for _ in range(50):
            ...
            if residual > 1.0e-4:
                raise ValueError(
                    f"{leg} Cartesian path is not reachable at progress "
                    f"{progress:.2f}: residual={residual:.6f} m"
                )
            if np.any(q_path < leg_low) or np.any(q_path > leg_high):
                raise ValueError(
                    f"{leg} Cartesian path leaves a joint limit at progress "
                    f"{progress:.2f}: q={np.rad2deg(q_path)} deg"
                )
            if path_singular < MIN_JACOBIAN_SINGULAR:
                raise ValueError(
                    f"{leg} Cartesian path approaches a singularity at "
                    f"progress {progress:.2f}: sigma_min="
                    f"{path_singular:.4f}"
                )
...
def run_hardware(
    ...
    print("[recorded] ROBOT MUST REMAIN MECHANICALLY SUPPORTED. X stops.")
...
def offline_self_test(stand_height=DEFAULT_STAND_HEIGHT, travel_scale=1.0):
    ...
    print("  Cartesian transition torque at t=0: zero")
    print(
